...
var $ = __webpack_require__(1); //EWW.

var _require = __webpack_require__(6),
    TweenMax = _require.TweenMax;

var Animations = function () {
  function Animations($el, colors) {
  ...
  _createClass(Animations, [{
    key: 'drawBlocksV2',
    value: function drawBlocksV2() {
      var id = parseInt(el.attr('data-id'));
    }
  }, {
    key: 'drawBlocks',
    value: function drawBlocks() {
      var animates = [{ value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }];
      var canvas = this.wrap.find("[data-id='canvas']")[0];
      var canvasId = this.wrap.find("[data-id='canvas']").attr('id');
      if (canvas.getContext) {
        // END OF FOREACH LOOP
        var _update = function _update(block) {
          ctx.clearRect(0, 0, canvas.width, canvas.height);
          if (realWidth <= that.maxThreshold) mobile = true;else mobile = false;
          that.points = [{ x: 0, y: that.offset }, // 0
          ...
          ctx.translate(20, 20);
          for (var _i = 0; _i < that.blocks.length; _i++) {
            var x = _i;
            if (_i > that.blocks.length / 2 - 1 && !mobile) x++;
            if (mobile) {
              switch (_i) {
                case 1:
                  {
            ...
            if (canvasId === 'canvas-home' && _i === 0) {
              var grd = void 0;
              if (!mobile) grd = ctx.createLinearGradient(0, 0, canvas.width * .25, 0);else grd = ctx.createLinearGradient(0, 0, canvas.width * .5, 0);
              grd.addColorStop(0, "green");
              grd.addColorStop(1, "green");
              ctx.fillStyle = that.blocks[_i];
            }
            if (canvasId === 'canvas-home' && _i === 1) {
              var _grd = void 0;
              if (!mobile) _grd = ctx.createLinearGradient(canvas.width * .25, 0, canvas.width * .5, 0);else _grd = ctx.createLinearGradient(canvas.width * .5, 0, canvas.width, 0);
              _grd.addColorStop(0, "green");
              _grd.addColorStop(0, "green");
              ctx.fillStyle = that.blocks[_i];
            } else ctx.fillStyle = that.blocks[_i];
        ...
        var _resize = function _resize() {
          var width = that.wrap.width() + 40;
          var height = that.wrap.height() + 40;
          canvas.width = width;
        ...
        this.wrap.find('.block').each(function () {
          ...
          var animate = animates[id];

          $(this).hover(function () {
            TweenMax.to(animate, .2, { value: 2, onUpdate: function onUpdate() {
                _update(id);
              } });
          }, function () {
            this.oldHover = id;
            TweenMax.to(animate, .2, { value: 0, onUpdate: function onUpdate() {
                _update(id);
              } });
          });
        });
